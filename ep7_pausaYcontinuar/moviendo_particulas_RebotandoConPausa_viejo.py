...
class Particula:
    ...
    def display(self):
        pygame.draw.circle(screen, red, (self.pos[0][0], self.pos[1][0]), self.rad, 1)
...
def g():
    #el modulo del campo gravitario es
    mod_g = 0.2 #m/s**2
    #la direccion del campo gravitatorio es
    u = np.array([[0],
                  [1]])
    g = mod_g * u
    return g
...
def P(obj):
    #el peso se calcula como la masa del objeto por el campo en el punto
    P = obj.m * g()
    return P
...
def sum_F(obj):
    ...
    #sum_F = m * a
    obj.a = sum_F / obj.m
    #Calculamos la velocidad de la particula sujeta a estas fuerzas
    #utilizando el metodo de Euler-Crommer
    #dv / dt = a
    #delta_v / delta_t = a
    #(vf - vi) = a * delta_t
