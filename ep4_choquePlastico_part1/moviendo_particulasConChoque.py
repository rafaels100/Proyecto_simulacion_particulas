...
def choque(partic1, partic2):
    ...
    if dist_part12 <= (partic1.rad + partic2.rad):
        ...
        rapi1tg = np.dot(partic1.v.T, tg)
        #calculo la proyeccion de la velocidad de la particula 1 sobre
        #el versor normal al que une a las dos particulas
        rapi1n = np.dot(partic1.v.T, n)
        #calculo la proyeccion de la velocidad de la particula 2 sobre
        #el versor tangente, que une a las dos particulas
        rapi2tg = np.dot(partic2.v.T, tg)
        #calculo la proyeccion de la velocidad de la particula 2 sobre
        #el versor normal al que une a las dos particulas
        rapi2n = np.dot(partic2.v.T, n)
        #Utilizando las ecuaciones del choque elastico, calculo la rapidez
        #final de la particula 1 y 2
        partic1.rapi = (2*partic2.masa/(partic1.masa+partic2.masa))*rapi2tg + \
                       ((partic1.masa-partic2.masa)/(partic1.masa+partic2.masa))*rapi1tg
        partic2.rapi = (2*partic1.masa/(partic1.masa+partic2.masa))*rapi1tg + \
                       ((partic2.masa-partic1.masa)/(partic1.masa+partic2.masa))*rapi2tg
        #Calculo las velocidades finales de la particula 1 y 2
        partic1.v = rapi1n * n + partic1.rapi * tg 
        partic2.v = rapi2n * n + partic2.rapi * tg
